...
describe("redactText", () => {
  it("redacts credentials and home paths before truncating output", () => {
    const givenOutput = [
      ...
      "/private/tmp/proofstack/project/report.json",
      "x".repeat(300),
    ].join("\n")

    const whenRedacted = redactText(givenOutput, {
      home: "/Users/alice",
      projectRoot: "/private/tmp/proofstack/project",
      maxLength: 320,
    })
    ...
    expect(whenRedacted).toContain("$PROJECT_ROOT/report.json")
